class UploadManager < BaseManager
  ...
  def create_video(transloadit_data, uploadable: user, template: 'post_video', attributes: {})
    ...
    thumbs.each do |thumb|
      preview = PendingVideoPreviewPhoto.new transloadit_data: pack_transloadit_data(thumb),
  ...
  # @param transloadit_data [ActionController::ManagebleParameters, Hash, Array]
  def pack_transloadit_data(transloadit_data)
    case transloadit_data
    when ActionController::ManagebleParameters
      transloadit_data.to_unsafe_hash.to_hash
    else
      transloadit_data.to_hash
    end
  end
