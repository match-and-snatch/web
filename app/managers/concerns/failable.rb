module Concerns::Failable
  ...
  def fail_with(message)
    ...
    when Symbol then errors[message] = t_error(:invalid)
    when Hash
      message.each do |k, v|
        case v
        when String then errors[k] = v
        when Symbol then errors[k] = t_error(v)
        when Hash
          messages = v.map { |translation_key, locals| t_error(translation_key, locals) }
