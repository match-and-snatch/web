...
describe UserProfileManager do
  ...
  describe '#update' do
    specify do
      expect { manager.update(subscription_cost: 1, slug: user.slug) }.not_to raise_error
    end
  ...
  describe '#update_payment_information' do
    ...
    context 'empty holder name' do
      specify do
        expect { manager.update_payment_information(holder_name: '') }.to raise_error(ManagerError) { |e| expect(e.messages).to have_key(:holder_name) }
      end
    end
