...
describe EventsManager do
  ...
  context 'sessions events' do
    ...
    describe '.user_registered' do
      it { expect(manager.user_registered(user: user)).to create_event(:registered) }
    end
  ...
  context 'comments events' do
    let(:comment) { CommentManager.new(user: another_user, post: _post).create(message: 'test') }

    before do
      SubscriptionManager.new(subscriber: another_user).subscribe_to(user)
    end
  ...
  context 'subscriptions events' do
    let(:subscription) { SubscriptionManager.new(subscriber: another_user).subscribe_to(user) }
