...
describe 'create_record' do
  ...
  it { expect { create :user }.to create_record(User).exactly(1.times) }
  it { expect { create :user }.to create_record(User).exactly(1) }
  it { expect { create :user }.to create_record(User).exactly('1 time in this spec') }
  it { expect {}.not_to create_record(User) }
  it { expect { create :user }.not_to create_record(User).exactly(2.times) }
  it { expect { create :user }.not_to create_record(User).exactly('2 times now') }
  it { expect { Post.create! }.not_to create_record(User) }

  it do
    expect do
      create :user
      create :user, email: 'another@email.com'
  ...
  it { expect { create :user, email: 'test@connectpal.com' }.to create_record(User).matching(email: 'test@connectpal.com').once }
  it { expect { create :user, email: 'test@connectpal.com' }.not_to create_record(User).matching(email: 'wrong@connectpal.com').once }
  it { expect { create :user, email: 'test@connectpal.com' }.to create_record(User.where(email: 'test@connectpal.com')) }
  it { expect { create :user, email: 'test@connectpal.com' }.not_to create_record(User.where(email: 'wrong@connectpal.com')) }
  ...
  context 'initial count > 0' do
    before { create :user }
    it { expect { create :user, email: 'another@email.com' }.to create_record(User).once }
  end
